...
if (jQuery) (function($) {
	...
	function _build(data) {
		...
		// Bind events
		// data.$zoomer.on("dblclick.zoomer", ":not(.zoomer-controls)", data, _zoomIn).on("touchend.zoomer mouseup.zoomer", data, _clearZoom);
	...
	function _dragStart(e) {
		...
		var data = e.data;
		data.action = "drag";
		var _zoomerImage = $(e.data.$zoomer).find('.zoomer-image');
		if(_zoomerImage) _zoomerImage.addClass('dragging');
	...
	function _dragStop(e) {
		...
		data.action = "";
		var _zoomerImage = $(e.data.$zoomer).find('.zoomer-image');
		if(_zoomerImage) _zoomerImage.removeClass('dragging');
	...
	function _onTouchEnd(data, oe) {
		...
		// Clear touch events
		if (data.touches.length < 1 || oe.preventDefault) {
			$(window).off(".zoomer");
...
jQuery(document).ready(function($) {
	jQuery('.cq-zoompan').each(function(index) {
		var _zoom = jQuery(this).css({
			...
			marginMax: 10,
			marginMax: $(this).data('marginmax'),
			marginMin: $(this).data('marginmin'),
			customClass: $(this).data('background') + '_zoomer'
	    })
	});
	$(window).on('resize', function() {
	   if($(this).width()>960){
			jQuery('.cq-zoompan').each(function(index) {
		       $(this).css('width', $(this).data('width')).zoomer("resize");
			})
	   }else{
	       $(".cq-zoompan").css('width', '100%').zoomer("resize");
	   }
    });
